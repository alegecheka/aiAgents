...
|----------|-------------------------------------|-----------------------------|
| string   | double quotes, one line             | `"hello"`                   |
| text     | triple quotes, multi-line, literal (see §4.2) | `"""first line`<br>`second line"""` |
| integer  | decimal                             | `-42`                       |
...
- Inside a text block **everything is literal**: newlines, backslashes, quotes,
  ...
  with one exception: the boundary-newline rule below.
- The one exception: to write a literal `"""` inside text, use `\"""`.
- The closing `"""` must be followed by the end of the field (then `;` or the
  closing brace).
- One leading newline right after the opening `"""` and one trailing newline
