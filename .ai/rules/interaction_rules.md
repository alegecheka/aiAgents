...
- **Additional Files:** The session folder MAY contain **up to 9 additional files** the agent deems useful for clarity (e.g., `agent-opinion.txt`, `notes.md`, `summary.html`, diagrams). Do **not** flood with boilerplate `README.md` files — keep it lean.
- **Agents is private:** Beyond this convention, **`agents/` content is not strictly linted** — each agent may use its own style and practices (see `agents/README.md`). Session folders are log stores, not projects, and are intentionally not hub-checked.
